...
def visualize():
    ...
    with torch.no_grad():
        ...
        #times = [0, 10, 40] #Use it when deal with task UN
        times = [0, 10, 30] #Use it when deal with task ACL
        topics_words = []
        ...
        #queries = ['africa', 'colonial', 'racist', 'democratic']
        try:
...
def get_topic_quality():
    ...
    with torch.no_grad():
        ...
        for tt in range(args.num_times):
            print('*'*10)
            print('Time number:',tt)
            tc, cnt, max_tc, tc_return = get_topic_coherence(beta[:, tt, :].cpu().numpy(), train_tokens, vocab)
            TC_all.append(tc)
            cnt_all.append(cnt)
            max_all.append(max_tc)
            topic_ch_41.append(tc_return)
        print()
        print('#'*100)
        print('TC_all: ', TC_all)
        TC = np.mean(TC_all)
        ...
        print('TC_all_size: ', TC_all.size())
        print("Filtered_TC:", max_all)
        Max = np.mean(max_all)
        #Max_tc = np.max(max_all)
        print("Mean_Filtered_TC:", Max)
        #print('Max_Max_TC:', Max_tc)
        print('TC_all: {}'.format(TC_all))
        print('\n')
        print('Get topic quality...')
        #quality = tc * diversity
        #quality = TC_all * TD
        quality = np.multiply(TC_all, TD)
        print('Topic Quality is: {}'.format(quality))
        mean_tq = TC * TD
        max_tq = Max * TD
        #Max_tq = Max_tc * TD
        print('Mean Topic Quality is: {}'.format(mean_tq))
        print('Filtered Topic Quality is: {}'.format(max_tq))
        #print('Max Max Topic Quality is: {}'.format(Max_tq))
        normal_topic_41 = []
        for tt in topic_ch_41:
            topic_41_piece = tt/45
            normal_topic_41.append(topic_41_piece)
        plt.plot(range(args.num_times), normal_topic_41)
        plt.xlabel('Time number')
        plt.ylabel('Topic Coherence')
        plt.title('Topic Coherence for Topic {}'.format(41))
        plt.savefig('TC_of_41.png')
        plt.show()
        print('#'*100)
